...
def benchmark_groupby(package_name, targ_size):
    ...
    scheduler_json_name = None
    
    dframe_package_dict = {'dask-cudf': (dask_cudf, 'my-scheduler-gpu.json'),
    ...
    package_handle, scheduler_json_name = dframe_package_dict[package_name]
    ...
    t0 = time.time()
    t_start = t0
    dframe = package_handle.read_csv('file://' + csv_patht)
    if scheduler_json_name is not None:
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print('please run as "python <script_name>.py <package> <size>"\n\t<package> = dask, dask-cudf, cudf or pandas\n\t<size> = 1G, 2.5G, 5G, ...25G')
    else:
        if sys.argv[2] == 'all':
            for dsize in ['1G', '2.5G', '5G' , '10G', '25G']:
                benchmark_groupby(sys.argv[1], dsize)
        else:
